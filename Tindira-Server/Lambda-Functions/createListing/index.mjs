...
export const handler = async (event) => {
  ...
  if (validateBodyHeadersQueryString(body) === false || username === undefined || username === "") { // Check that body/header params for the listing have been passed correctly
    const response = {
      statusCode: 400,
      headers: {
  ...
  try {
    await createListing(body, listingId); // Upload the listing
  } catch (err) {
