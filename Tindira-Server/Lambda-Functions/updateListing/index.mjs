...
export const handler = async (event) => {
  ...
  if (validateBodyHeadersQueryString(body) === false) { // Check that body/header params for the listing have been passed correctly
    const response = {
      statusCode: 400,
      headers: {
...
function validateBodyHeadersQueryString(body) { // Use Joi to create a schema for the validation of the request body that stores the listing

  /*if (!headers || headers.username === undefined || headers.username === "" || headers.token === undefined || headers.token === "") {
    return false;
  }*/

  const listingIdSchema = Joi.object().keys({
  ...
  const { error2, value2 } = listingIdSchema.validate(body); // Validate the listing ID input
  if (error2) {
    return false;
  }
