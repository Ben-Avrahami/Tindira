...
export const handler = async (event) => {
  ...
  let updated = await updateListing(body, pricePerMonth, pricePerWholeTime); // Upload the listing

  if (updated === "error") { // Errors uploading the listing
    const response = {
      statusCode: 500,
      headers: {
        "Access-Control-Allow-Origin": "*",
      ...
      body: JSON.stringify([body.listingId, body.title])
    };
...
function validateBodyHeaders(body) { // Use Joi to create a schema for the validation of the request body that stores the listing
  ...
  const listingSchema = Joi.object().keys({
    ...
    contractStartingDate: Joi.date().format('YYYY-MM-DD'),
...
async function updateListing(body, pricePerMonth, pricePerWholeTime) {
  ...
  Object.keys(bodyUpdated).forEach(function(key) {
    if (key !== 'listingId') {
      ...
      expressionAttributeValues[":" + key] = marshall(bodyUpdated[key]); // assuming a is a string, change as necessary
    }
  });
